    ...
    private void setupButtons() {
        ...
        if (btnSaveJob != null) {
            btnSaveJob.setOnClickListener(v -> {
                ...
                btnSaveJob.setText("Sauvegardé");
            });
        }

        // View company button
        Button btnViewCompany = findViewById(R.id.btnViewCompany);
        if (btnViewCompany != null) {
            btnViewCompany.setOnClickListener(v -> {
                Toast.makeText(this, "Page entreprise en développement", Toast.LENGTH_SHORT).show();
            });
        }

        // Back button (collapsing toolbar)
